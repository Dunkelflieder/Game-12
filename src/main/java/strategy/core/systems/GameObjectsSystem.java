...
public class GameObjectsSystem extends SynchronizedSystem {
	...
	public void init() {
		if (checkSide(Side.SERVER)) {
			loadObjectsFromFolder(Strategy.DATA_DIR + "objects");
			initObjectIDs();
		}
	}
	...
	private Component createComponent(short entityID, String componentName, NDSNodeObject data) {
		try {
			...
			Strategy.logger.log(Logger.ERROR, "could not set data for component: " + componentName + " in entity " + getName(entityID));
			e.printStackTrace(Strategy.logger.getErrorStream());
			return null;
		}
	}
