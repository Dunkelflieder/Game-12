...
import strategy.server.map.ServerMapLoader;
...
public class ServerMainThread extends Thread {
	...
	public ServerMainThread() {
		try {
			...
			networkAdapter = serverThread.getNetworkAdapter(Strategy.NETWORK_ADAPTER_DEFAULT);
	...
	private void startCommand(String mapID) {
		if (currentMaps != null) {
			Strategy.logger.log(Logger.WARNING, "start packet received while game is running");
		}
		...
		for (Connection connection : connections) {
			factions[0].addNetworkAdapter(connection.getNetworkAdapter(Strategy.NETWORK_ADAPTER_DEFAULT));
		}
	...
	private void stopCommand() {
		Strategy.logger.log(Logger.WARNING, "stop command received, not implemented yet");
	}

	private void pauseCommand() {
		Strategy.logger.log(Logger.WARNING, "pause command received, not implemented yet");
	}

	private void resumeCommand() {
		Strategy.logger.log(Logger.WARNING, "resume command received, not implemented yet");
	}
