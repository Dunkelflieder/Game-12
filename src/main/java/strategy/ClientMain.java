...
import strategy.client.Menu;
import strategy.client.PerformanceProfiler;
import strategy.client.event.BeforeRenderEvent;
import strategy.client.event.ClientChangeEvent;
...
public class ClientMain {
	...
	private static void initLib() {
		...
		// logger
		Strategy.logger.setParent(Noise.getLogger());
	...
	private static void init() {
		...
		try {
			...
			Strategy.logger.log(Logger.WARNING, "Could not create openCL context");
		}
	...
	private static void run() {
		...
		while (!window.shouldClose()) {
			...
			if (timer.getCalcTime() > 0.1) Strategy.logger.log(Logger.INFO, "Long frame time: " + timer.getCalcTime() + "s");
		}
